...
```{r}
library(gdata)
resamples = sapply(1:10000, function(i) {
  res = sample(x, replace = TRUE)
  return((mean(res) - mean(x))/se(res))
})
```
...
```{r}
x2 = dataset[,2]
resamples2 = sapply(1:10000, function(i) {
  res = sample(x2, replace = TRUE)
  return((mean(res) - mean(x2))/se(res))
})
```
```{r}
...
options(warn=-1)
# read in csv "WPP2010.csv" and name it WPP2010
...
# change the name from column 7th to 17th to age0 to age50, interval as 5
...
newdataset = dataset[dataset$year <= 1970,]
...
```{r}
## a
p1 <- (ccso %>% filter(Age.at.Arrest >= 17) %>% filter(Age.at.Arrest <= 23)  %>% filter(STATE != "ILLINOIS") %>% filter(as.Date(BOOKING.DATE, "%m/%d/%Y") >= "2012/01/01") %>% filter(as.Date(BOOKING.DATE, "%m/%d/%Y") < "2013/01/01"))
dim(p1)
```
```{r, eval= FALSE}
## b
p2 <- (ccso %>% filter(EMPLOYMENT.STATUS == "Student") %>% filter(CITY == "DANVILLE") %>% filter(as.Date(BOOKING.DATE, "%m/%d/%Y") >= "2013/01/01"))
dim(p2)
```
```{r, eval= FALSE}
## c
p3 <- (ccso %>% filter(CITY == "CHAMPAIGN" | CITY == "URBANA") %>% filter(RACE == "Asian/Pacific Islander"))
dim(p3)
```
